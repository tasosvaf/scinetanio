...
from utils.utils_ETTh import getGreekScinetInputDim
...
class Exp_financial(Exp_Basic):
    ...
    def _build_model(self):
        if self.args.dataset_name.startswith("greek_scinet_dataset_"):
            self.input_dim = getGreekScinetInputDim(self.args.dataset_name)
    ...
    def _get_data(self):
        if self.args.dataset_name.startswith("greek_scinet_dataset_"):
            self.args.data = f"./datasets/financial/{self.args.dataset_name}.txt" 
        ...
        # 2010 - 2016 training : Rows[2-54817] -> 54816 rows ~= 0.7577969
        # 2017 validation  : Rows[54818-63577] -> 8760 rows  ~= 0.1211015
        # 2018 test        : Rows[63578-72337] -> 8760 rows  ~= 0.1211015
        # Total rows: 72336
            
        training_ratio = 0.7578
        validation_ratio = 0.121103

        if self.args.long_term_forecast:
            return DataLoaderH(self.args.data, 0.7, 0.1, self.args.horizon, self.args.window_size,self.args.future_unknown_days, 4)
        else:
            return DataLoaderH(self.args.data, training_ratio, validation_ratio, self.args.horizon, self.args.window_size, self.args.future_unknown_days, self.args.normalize)

    def _select_optimizer(self):
        return torch.optim.Adam(params=self.model.parameters(), lr=self.args.lr, betas=(0.9, 0.999), weight_decay=1e-5)


    def train(self, train_pred_file_path):
        ...
        if self.args.resume:
            self.model, lr, epoch_start = load_model(self.model, save_path, model_name=str(self.args.future_unknown_days) + self.args.dataset_name, horizon=self.args.horizon)
        else:
            epoch_start = 0
        ...
        for epoch in range(epoch_start, self.args.epochs):
            ...
            for tx, ty in data.get_batches(X, Y, self.args.batch_size, True):
                ...
                iter += 1
            if self.args.stacks == 1:
                predict_val, Yval, val_loss, val_rae, val_corr, val_mse, val_mae = self.validate(data, data.valid[0],data.valid[1])
                predict_test, Ytest, test_loss, test_rae, test_corr, test_mse, test_mae = self.validate(data, data.test[0],data.test[1])      
            elif self.args.stacks == 2:
                predict_val, Yval, val_loss, val_rae, val_corr, val_rse_mid, val_rae_mid, val_correlation_mid, val_mse, val_mae =self.validate(data, data.valid[0],data.valid[1])
                predict_test, Ytest, test_loss, test_rae, test_corr, test_rse_mid, test_rae_mid, test_correlation_mid, test_mse, test_mae = self.validate(data, data.test[0],data.test[1])
            ...
            true_test = []
            true_val = []
            predictions_val = predict_val * data.get_max()
            y_val = Yval * data.get_max()
            predictions_test = predict_test * data.get_max()
            y_test = Ytest * data.get_max()
            for i in range(0, self.args.groups):
                pred_val.append([])
                pred_test.append([])
            for predict in predictions_val:
                for i in range(0, self.args.groups):
                    pred_val[i].append(predict[i])
            for predict in predictions_test:
                for i in range(0, self.args.groups):
                    pred_test[i].append(predict[i])
            for i in range(0, self.args.groups):
                df[str(epoch) + '_epoch_pred_val' + str(i)] = pred_val[i]
                df[str(epoch) + '_epoch_pred_test' + str(i)] = pred_test[i]
            for tr in y_test:
                true_test.append(tr[0])
            for tr in y_val:
                true_val.append(tr[0])
            df[str(epoch) + '_epoch_true_val'] = true_val
            df[str(epoch) + '_epoch_true_test'] = true_test
            
            if epoch == 0:
                df.to_csv(train_pred_file_path, index = False)
            else:
                old_df = pd.read_csv(train_pred_file_path)
                new_df = pd.concat([old_df, df], axis = 1)
                new_df.to_csv(train_pred_file_path, index = False)
            ...
            if val_mse < best_val and self.args.long_term_forecast:
                save_model(epoch, lr, self.model, save_path, model_name=str(self.args.future_unknown_days) + self.args.dataset_name, horizon=self.args.horizon)
                print('--------------| Best Val loss |--------------')
                best_val = val_mse
    ...
    def validate(self, data, X, Y, evaluate=False):
        ...
        if evaluate:
            save_path = os.path.join(self.args.save_path, self.args.model_name)
            self.model = load_model(self.model, save_path, model_name=str(self.args.future_unknown_days) + self.args.dataset_name, horizon=self.args.horizon)[0]
        ...
        if self.args.stacks == 1:
            return predict, Ytest, rse, rae, correlation, mse, mae
        if self.args.stacks == 2:
            return predict, Ytest, rse, rae, correlation, rse_mid, rae_mid, correlation_mid, mse, mae
