...
import warnings
from utils.utils_ETTh import metric
from utils.utils_ETTh import calculateAlgorithmMetrics
from utils.utils_ETTh import calculateMyMetricsFromDenormalizedValuesUsingFilePathInfo
from utils.utils_ETTh import EpochMetrics
...
class Exp_ETTh(Exp_Basic):
    ...
    def _build_model(self):
        if self.args.features == 'S':
            in_dim = 1
        elif self.args.features == 'M' or self.args.features == 'MS':
            if self.args.data in ["greek_energy", "ETTh1", "ETTh2"]:
                in_dim = self.args.num_features
            else: 
                in_dim = 7
        else:
            print('Error!')

        if self.args.decompose == 'Yes':
    ...
    def _get_data(self, flag):
        ...
        data_set = Data(
            ...
            freq=freq,
            cols=args.cols,
            future_unknown_days = args.future_unknown_days,
            shift_data_y = args.shift_data_y,
            anio = args.anio
        )
    ...
    def valid(self, valid_data, valid_loader, criterion):
        ...
        if self.args.stacks == 1:
            ...
            correct_predictions = pred_scales[pred_len-1::pred_len,:,-1].flatten()
            correct_true = true_scales[pred_len-1::pred_len,:,-1].flatten()
            
            correct_mid_predictions = None
            mid_scales = None
            ...
            pred_len = self.args.pred_len
            correct_mid_predictions = mid_scales[pred_len-1::pred_len,:,-1].flatten()
            correct_predictions = pred_scales[pred_len-1::pred_len,:,-1].flatten()
            correct_true = true_scales[pred_len-1::pred_len,:,-1].flatten()
        ...
        return total_loss, correct_predictions, correct_true, correct_mid_predictions, pred_scales, true_scales, mid_scales

    def train(self, setting, pred_file_path, metrics_file_path):
        ...
        for epoch in range(epoch_start, self.args.train_epochs):
            ...
            valid_loss, valid_pred, valid_true, valid_mid_pred, valid_pred_scales, valid_true_scales, valid_mid_scales = self.valid(valid_data, valid_loader, criterion)
            print('--------start to test-----------')
            test_loss, test_pred, test_true, test_mid_pred, test_pred_scales, test_true_scales, test_mid_scales = self.valid(test_data, test_loader, criterion)
            #Saving results to pred_file_path
            df = pd.DataFrame()
            if valid_mid_pred is not None:
                df[str(epoch) + '_epoch_mid_pred_valid'] = valid_mid_pred
                df[str(epoch) + '_epoch_mid_pred_test'] = test_mid_pred
            df[str(epoch) + '_epoch_pred_valid'] = valid_pred
            df[str(epoch) + '_epoch_pred_test'] = test_pred
            df[str(epoch) + '_epoch_true_valid'] = valid_true
            df[str(epoch) + '_epoch_true_test'] = test_true
            
            valid_algorithm_metrics_full, valid_algorithm_metrics, valid_algorithm_metrics_red = calculateAlgorithmMetrics(
                valid_pred_scales, valid_true_scales, valid_mid_scales, 
                valid_pred, valid_true, valid_mid_pred, pred_file_path)
            
            test_algorithm_metrics_full, test_algorithm_metrics, test_algorithm_metrics_red = calculateAlgorithmMetrics(
                test_pred_scales, test_true_scales, test_mid_scales, 
                test_pred, test_true, test_mid_pred, pred_file_path)
            
            if epoch == 0:
                df.to_csv(pred_file_path, index = False) 
            else:
                old_df = pd.read_csv(pred_file_path)
                new_df = pd.concat([old_df, df], axis = 1)
                new_df.to_csv(pred_file_path, index = False)
            
            valid_my_metrics = calculateMyMetricsFromDenormalizedValuesUsingFilePathInfo(valid_pred, valid_true, pred_file_path, 'valid')
            test_my_metrics = calculateMyMetricsFromDenormalizedValuesUsingFilePathInfo(test_pred, test_true, pred_file_path, 'test')
            
            metrics = EpochMetrics(
                valid_algorithm_metrics_full = valid_algorithm_metrics_full, valid_algorithm_metrics = valid_algorithm_metrics, valid_algorithm_metrics_red = valid_algorithm_metrics_red,
                test_algorithm_metrics_full= test_algorithm_metrics_full, test_algorithm_metrics = test_algorithm_metrics, test_algorithm_metrics_red = test_algorithm_metrics_red,
                valid_my_algorithm_metrics = valid_my_metrics, test_my_algorithm_metrics = test_my_metrics)
            if False:
                print(f"Epoch: {epoch}")
                print("Epoch Metrics:")
                metrics.print()
            if epoch == 0:
                df = pd.DataFrame(metrics.getDictForDataFrame())
                df.to_csv(metrics_file_path, index = False, header = True)
            else:
                old_df = pd.read_csv(metrics_file_path)
                new_df = pd.DataFrame(metrics.getDictForDataFrame())
                new_df = pd.concat([old_df, new_df], axis = 0)
                new_df.to_csv(metrics_file_path, index = False, header = True)                      
