...
class DataLoaderH(object):
    ...
    def __init__(self, file_name, train, valid, horizon, window, future_unknown_days, normalize=2):
        self.P = window
        self.h = horizon
        fin = open(file_name)
        ...
        self.max = self.scale[0]
        print(int(train * self.n), int((train + valid) * self.n), self.n)
    ...
    def _batchify(self, idx_set, horizon):
        idx_missing_days = self.future_unknown_days * self.h
        n = len(idx_set) - idx_missing_days
        ...
        for i in range(n):
            ...
            Y[i, :, :] = torch.from_numpy(self.dat[idx_missing_days + end:idx_missing_days + (idx_set[i]+1), :])
